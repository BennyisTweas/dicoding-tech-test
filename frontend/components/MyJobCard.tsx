...
export default function MyVacancyCard({ vacancy }: { vacancy: Vacancy }) {
    return (
        <div className="w-[730px] h-[128px] border p-[16px] rounded-[1px] grid gap-[10px] items-center">
            <div key={vacancy.id} id="vacancy" className="relative flex gap-[16px]">
                ...
                <div id="card-element" className="flex flex-col gap-[16px] pb-2 text-[#18181B]">
                    ...
                    <div id="card-info" className="flex gap-[8px] text-xs font-normal text-[#52525B]">
                        <div>{"Dibuat: "}{formatDate(vacancy.job_created)}</div>
                        <div>{"Aktif hingga "}{formatDate(vacancy.job_closed_registration)}</div>
